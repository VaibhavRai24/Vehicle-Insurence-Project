...
class SimpleStorageService:
    ...
    def get_bucket(self, bucket_name:str) ->Bucket:
        ...
        try:
            ...
            logging.info("Exited the get_bucket method of SimpleStorgaeService")
            return bucket
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def get_file_object(self, filename:str, bucket_name:str) -> Union[List[object], object]:
        ...
        """
        logging.info("Entered the get_file_object method of SimpleStorageServices class")
        try:
            bucket = self.get_bucket(bucket_name)
            file_object = [file_object for file_object in bucket.objects.filter(Prefix=filename)]
            func = lambda x:x[0] if len(x) ==1 else x
            file_objs = func(file_object)
            logging.info("Exited the get_file_object method of SimpleStorageServices class")
            return file_objs
        except Exception as e:
            raise MyException(e, sys) from e
