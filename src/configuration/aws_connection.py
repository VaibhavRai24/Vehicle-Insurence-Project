...
class S3Client:
    ...
    def __init__(self, region_name = REGION_NAME):
        ...
        """
        if S3Client.s3_resource == None or S3Client.s3_client ==0:
            __access_key_id = os.getenv(AWS_ACCESS_KEY_ID_ENV_KEY)
            __secret_access_key = os.getenv(AWS_SECRET_ACCESS_KEY_ENV_KEY)
            if __access_key_id is None:
                raise Exception (f" Enviromental variable : {AWS_ACCESS_KEY_ID_ENV_KEY} is not set")
            if __secret_access_key is None:
                raise Exception (f" Eniromental variable {AWS_SECRET_ACCESS_KEY_ENV_KEY} is not set.")
